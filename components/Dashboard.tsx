    ...
    <div className="space-y-8">
                 ...
                 <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl overflow-x-auto scrollbar-hide">
                   {(['DAILY', 'WEEKLY', 'MONTHLY', 'YEARLY', 'CUSTOM'] as const).map(filter => (
                     <button
                       ...
                       className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all whitespace-nowrap active:scale-95 ${
                         activeFilter === filter 
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-950/70 backdrop-blur-sm transition-opacity" onClick={() => setIsAddModalOpen(false)}></div>
          <div className="relative w-full max-w-md glass-card rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-pop-in">
            ...
            <div className="p-8 pb-0 shrink-0">
               <h3 className="text-2xl font-bold text-slate-800 dark:text-white text-center">Tambah Transaksi</h3>
            </div>

            <div className="p-8 pt-6 overflow-y-auto scrollbar-hide">
              <form onSubmit={handleAddSubmit} className="space-y-5">
                <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800/50 rounded-2xl">
                  <button type="button" onClick={() => { setType('INCOME'); }} className={`flex-1 py-3 rounded-xl text-[10px] sm:text-xs font-bold transition-all duration-300 hover:scale-105 active:scale-95 ${type === 'INCOME' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>MASUK</button>
                  <button type="button" onClick={() => { setType('EXPENSE'); }} className={`flex-1 py-3 rounded-xl text-[10px] sm:text-xs font-bold transition-all duration-300 hover:scale-105 active:scale-95 ${type === 'EXPENSE' ? 'bg-white dark:bg-slate-700 text-red-600 dark:text-red-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>KELUAR</button>
                  <button type="button" onClick={() => { setType('TRANSFER'); }} className={`flex-1 py-3 rounded-xl text-[10px] sm:text-xs font-bold transition-all duration-300 hover:scale-105 active:scale-95 ${type === 'TRANSFER' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>TRANSFER</button>
                </div>
                ...
                <div>
                  ...
                  <select value={walletId} onChange={(e) => setWalletId(e.target.value)} className="w-full px-5 py-4 border border-slate-200 dark:border-slate-700/50 rounded-3xl text-sm bg-slate-50 dark:bg-slate-900/50 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all appearance-none cursor-pointer font-medium text-slate-900 dark:text-white">
                ...
                {type === 'TRANSFER' && (
                  <div className="relative animate-scale-in">
                    ...
                    <select value={toWalletId} onChange={(e) => setToWalletId(e.target.value)} className="w-full px-5 py-4 border border-slate-200 dark:border-slate-700/50 rounded-3xl text-sm bg-slate-50 dark:bg-slate-900/50 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all appearance-none cursor-pointer font-medium text-slate-900 dark:text-white">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" required value={amount} onChange={handleAmountChange} className="w-full px-5 py-4 border border-slate-200 dark:border-slate-700/50 rounded-3xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none font-bold text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-900/50" placeholder="0" />
                  </div>
                  <div>
                     <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Kategori</label>
                     <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-5 py-4 border border-slate-200 dark:border-slate-700/50 rounded-3xl text-sm bg-slate-50 dark:bg-slate-900/50 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none appearance-none font-medium text-slate-900 dark:text-white">
                ...
                <div>
                  ...
                  <input type="text" required value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-5 py-4 border border-slate-200 dark:border-slate-700/50 rounded-3xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none font-medium text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-900/50" placeholder="Cth: Beli Kopi / Transfer Tabungan" />
                </div>

                <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Tanggal</label>
                  <input type="date" required value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-5 py-4 border border-slate-200 dark:border-slate-700/50 rounded-3xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none font-medium text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-900/50" />
                </div>

                <div className="flex space-x-3 pt-4">
                  <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-4 text-sm font-bold text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-3xl transition-colors hover:scale-105 active:scale-95">Batal</button>
                  <button type="submit" className="flex-1 py-4 bg-indigo-600 text-white rounded-3xl text-sm font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:shadow-indigo-300 transition-all transform hover:-translate-y-0.5 hover:scale-105 active:scale-95">Simpan</button>
                </div>
              </form>
            </div>
          </div>
