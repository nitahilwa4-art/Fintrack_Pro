...
import { CreditCard, Plus, Trash2, Smartphone, Landmark, Banknote, Edit2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
          ...
          <div key={w.id} className={`relative rounded-[1.5rem] shadow-xl overflow-hidden text-white transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl h-56 flex flex-col justify-between p-6 ${getGradient(w.type)} group`}>
            ...
            <div className="relative z-10 flex justify-between items-start">
               ...
               <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-all">
                  <button onClick={() => openEditModal(w)} className="text-white/50 hover:text-white p-1 rounded-full hover:bg-white/20 transition-all">
                      <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => onDelete(w.id)} className="text-white/50 hover:text-white p-1 rounded-full hover:bg-white/20 transition-all">
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-md p-8 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 border border-slate-200 rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none font-medium text-slate-900 bg-slate-50" placeholder="Cth: BCA Utama, GoPay" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Tipe</label>
                  <select value={type} onChange={(e) => setType(e.target.value as any)} className="w-full px-4 py-3 border border-slate-200 rounded-2xl text-sm bg-slate-50 focus:bg-white outline-none cursor-pointer font-medium text-slate-900">
                ...
                <div>
                  ...
                  <input type="number" required value={initialBalance} onChange={(e) => setInitialBalance(e.target.value)} className="w-full px-4 py-3 border border-slate-200 rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none font-medium text-slate-900 bg-slate-50" placeholder="0" />
                </div>
              </div>
      ...
      {isEditOpen && editItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] w-full max-w-md p-8 shadow-2xl">
            <h3 className="text-2xl font-bold text-slate-800 mb-6">Edit Dompet</h3>
            <form onSubmit={handleEditSubmit} className="space-y-5">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Nama Dompet</label>
                <input type="text" required value={editItem.name} onChange={(e) => setEditItem({...editItem, name: e.target.value})} className="w-full px-4 py-3 border border-slate-200 rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none font-medium text-slate-900 bg-slate-50" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Tipe</label>
                  <select value={editItem.type} onChange={(e) => setEditItem({...editItem, type: e.target.value as any})} className="w-full px-4 py-3 border border-slate-200 rounded-2xl text-sm bg-slate-50 focus:bg-white outline-none cursor-pointer font-medium text-slate-900">
                    <option value="CASH">Tunai</option>
                    <option value="BANK">Bank</option>
                    <option value="E-WALLET">E-Wallet</option>
                  </select>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Saldo (Adjustment)</label>
                  <input type="number" required value={editItem.balance} onChange={(e) => setEditItem({...editItem, balance: parseFloat(e.target.value) || 0})} className="w-full px-4 py-3 border border-slate-200 rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none font-medium text-slate-900 bg-slate-50" />
                </div>
              </div>
              <div className="flex space-x-3 pt-6">
                ...
                <button type="submit" className="flex-1 py-3.5 bg-indigo-600 text-white rounded-2xl text-sm font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-colors">Simpan Perubahan</button>
              </div>
            </form>
          </div>
        </div>
